// AUTO-CURATED — one ayah at a time, for learning together.
// Arabic text: Uthmani (api.quran.com, verse key exact).
// Translation: Saheeh International.
// Every entry: idea = the ONE thing to take away. ask = open question (never the answer).
// trythis = something to do today. papa = context for the adult. sources = primary only.
window.AYAT_POSTS = [
 {
  "date": "2026-09-25",
  "ref": "49:13",
  "emoji": "🌍",
  "color": "#4FC3F7",
  "arabic": "يَـٰٓأَيُّهَا ٱلنَّاسُ إِنَّا خَلَقْنَـٰكُم مِّن ذَكَرٍ وَأُنثَىٰ وَجَعَلْنَـٰكُمْ شُعُوبًا وَقَبَآئِلَ لِتَعَارَفُوٓا۟ ۚ إِنَّ أَكْرَمَكُمْ عِندَ ٱللَّهِ أَتْقَىٰكُمْ ۚ إِنَّ ٱللَّهَ عَلِيمٌ خَبِيرٌ",
  "translation": "O mankind, indeed We have created you from male and female and made you peoples and tribes that you may know one another. Indeed, the most noble of you in the sight of Allāh is the most righteous of you. Indeed, Allāh is Knowing and Aware.",
  "translation_source": "Saheeh International",
  "idea": "Allah did not make people different so we could rank them. In this ayah the only thing that raises a person is being careful to do right.",
  "ask": "If the most noble person is the one most careful to do right, what should you look at when you meet someone new?",
  "hint": "Is it what they look like, or what they do?",
  "trythis": "Today find someone who seems different from you — older, younger, speaks another language. Learn one real thing about them.",
  "papa": "Ibn Kathir: all mankind are the children of Adam and Hawwa'; nations and tribes exist so that people may know one another, not so they may boast. The verse names taqwa (being careful of Allah) as the only measure of superiority — no lineage, wealth or appearance is mentioned.",
  "sources": ["Qur'an 49:13 — Uthmani text (api.quran.com)", "Saheeh International translation", "Tafsir Ibn Kathir (abridged), 49:13"]
 },
 {
  "date": "2026-09-25",
  "ref": "Al-Isra' 17:23",
  "emoji": "🌙",
  "color": "#81C784",
  "arabic": "۞ وَقَضَىٰ رَبُّكَ أَلَّا تَعْبُدُوٓا۟ إِلَّآ إِيَّاهُ وَبِٱلْوَٰلِدَيْنِ إِحْسَـٰنًا ۚ إِمَّا يَبْلُغَنَّ عِندَكَ ٱلْكِبَرَ أَحَدُهُمَآ أَوْ كِلَاهُمَا فَلَا تَقُل لَّهُمَآ أُفٍّ وَلَا تَنْهَرْهُمَا وَقُل لَّهُمَا قَوْلًا كَرِيمًا",
  "translation": "And your Lord has decreed that you worship not except Him, and to parents, good treatment. Whether one or both of them reach old age [while] with you, say not to them [so much as], \"uff,\" and do not repel them but speak to them a noble word.",
  "translation_source": "Saheeh International",
  "idea": "\"Uff\" is the smallest sound that slips out when we are annoyed. Even that tiny sound is not allowed. So if the smallest one is not allowed, the bigger ones are even more not allowed.",
  "ask": "Why do you think Allah mentions the smallest sound — not a shout, not a big word?",
  "hint": "Think about what comes first when someone gets annoyed. Does it start big or small?",
  "trythis": "Today, notice the moment the little \"uff\" wants to come out. Swap it for one kind sentence instead. Count how many times you did it.",
  "papa": "Three steps in one verse: do not say \"uff\" → do not repel them → speak a noble word. Small negative, bigger negative, then the positive command. Ibn Kathir calls \"uff\" the mildest word of disrespect — that is the boundary being set. The word \"'indaka\" (in your care) marks the moment parents are old and dependent on you; the command lands hardest exactly there. In Bukhari, after prayer at its time, kindness to parents is named as the next most favoured deed.",
  "sources": [
   "Qur'an 17:23 — Uthmani text (api.quran.com)",
   "Saheeh International translation",
   "Tafsir Ibn Kathir (abridged), 17:23",
   "Sahih al-Bukhari — kindness to parents ranked after prayer at its time"
  ]
 }
];
window.AYAT_LATEST = window.AYAT_POSTS[0];